...
Applet::Applet(QObject *parent, const QVariantList &args)
		: Plasma::Applet(parent, args),
		  ...
		  m_groupManager(new MyGroupManager(this)),
		  m_rootGroup(0),
...
Applet::~Applet() {
	...
	disconnect(
        ...
        disconnect(m_rootGroup, NULL, this, NULL);
...
void Applet::init() {
	...
	setLayout(m_layout);
	connect( m_layout, SIGNAL(sizeHintChanged(Qt::SizeHint)),
...
void Applet::itemAboutToBeRemoved(AbstractGroupableItem *)
{
}

void Applet::connectRootGroup() {
	TaskManager::TaskGroup *group = m_rootGroup;
	qDebug()<< "reload connectRootGroup";
	if (group) {
		connect(
...
// 		connect(group, SIGNAL(itemAboutToBeAdded(AbstractGroupableItem*,int)), 
// 		      this,SLOT(itemAboutToBeAdded(AbstractGroupableItem*,int)));
// 		
// 		connect(group, SIGNAL(itemAboutToBeRemoved(AbstractGroupableItem*)), 
// 		      this,SLOT(itemAboutToBeRemoved(AbstractGroupableItem*)));
	}
}

void Applet::itemAdded(AbstractGroupableItem* groupableItem) {
	qDebug("itemAdded: 0x%lx \"%s\" \"%s\".. [%c]", (unsigned long) groupableItem, qPrintable(groupableItem->name()), qPrintable(groupableItem->launcherUrl().prettyUrl()), groupableItem->isGrouped());
	...
	qDebug("itemAdded: index=%d",index);
...
void Applet::itemRemoved(AbstractGroupableItem* groupableItem) {
	qDebug("itemRemoved: 0x%lx \"%s\"  \"%s\"", (unsigned long) groupableItem, qPrintable(groupableItem->name()), qPrintable(groupableItem->launcherUrl().prettyUrl()));
...
void Applet::createConfigurationInterface(KConfigDialog *parent) {
	...
	m_configA.animationDuration->setValue(m_layout->duration());
...
void Applet::configAccepted() {
	...
	if (m_configA.animationDuration->value() != m_layout->duration()) {
...
void Applet::configuration() {	
	...
	if (cfgTaskbarLayout != m_layout->type()) {
		...
		newLayout->setExpandDuration(m_layout->duration());
