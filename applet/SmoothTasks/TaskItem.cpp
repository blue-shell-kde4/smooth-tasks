...
TaskItem::TaskItem(AbstractGroupableItem *abstractItem, Applet *applet)
	...
	if (m_task->type() == Task::StartupItem) {
		...
		m_light->startAnimation(Light::StartupAnimation, 600);
	}
...
void TaskItem::updateState() {
	...
	if (m_task->demandsAttention()) {
		...
		m_light->startAnimation(Light::AttentionAnimation, 600);
	} 
	...
	m_stateAnimation.setState(newState, m_applet->animationDuration());
}
...
void TaskItem::dragMoveEvent(QGraphicsSceneDragDropEvent *event) {
	...
	qDebug()<< "QGraphicsSceneDragDropEvent" << event->pos();
...
void TaskItem::paint(QPainter *p, const QStyleOptionGraphicsItem *option, QWidget *widget) {
	...
	//qDebug() << bounds.left() << ' ' << bounds.top() << ' ' << bounds.width() << ' ' << bounds.height();
	...
	if (isVertical) 
	{
		lightBounds.setWidth(bounds.height());
		lightBounds.setHeight(bounds.width());
	...
	if (m_applet->lights() && m_task->type() != Task::LauncherItem) {
		...
		m_light->paint(p, lightBounds, pos, mouseIn, isVertical); //lightBounds
	}
