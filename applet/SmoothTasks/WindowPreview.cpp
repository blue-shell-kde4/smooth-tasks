...
//	m_background->setImagePath("widgets/button");
	...
	connect( task, SIGNAL(changed(::TaskManager::TaskChanges)),
		this, SLOT(updateTask(::TaskManager::TaskChanges)));

	connect(&m_highlite, SIGNAL(update()), this, SLOT(repaint()));
}
...
void WindowPreview::setClassicLayout() {
	...
	// task name:
 	m_taskNameLabel = new FadedText(m_task->text(), NULL);
...
void WindowPreview::hoverEnter() {
	...
	m_highlite.startUp(applet->animationDuration());
	m_hover = true;
	emit enter(this);
...
void WindowPreview::hoverLeave() {
	Applet *applet = m_toolTip->applet();
	m_highlite.startDown(applet->animationDuration());
	m_hover = false;
	if (m_activateTimer) {
		delete m_activateTimer;
		m_activateTimer = NULL;
