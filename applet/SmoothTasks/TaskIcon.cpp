...
//          qDebug() << "actualSize " << m_icon.actualSize(QSize(size(),size()));
...
//         qDebug() << "TaskIcon size" << size <<  " (" << m_rect.width() << ',' << m_rect.height() << ") " <<  m_item->applet()->iconScale();
...
void TaskIcon::startStartupAnimation(int duration) {
	m_currentAnimationDuration = duration;
        m_step =  (qreal)1/m_currentAnimationDuration;
	setLoopCount(-1);
        start();
}
...
void TaskIcon::updateCurrentTime(int currentTime) {
...
// 	qDebug() << "animationStartup " << progress << ' ' << width << ' ' << height;
