...
void TaskbarLayout::moveDraggedItem(const QPointF& pos) {
	...
	qDebug() << "moveDraggedItem" << m_draggedItem->destX << m_draggedItem->destY << pos;
	m_mouseIn = true;
	...
	//m_draggedItem->start = m_draggedItem->item->rect();
...
void TaskbarLayout::animate(TaskbarItem *item, qreal move, qreal expand) {
	...
	qDebug() << item << rect << "|" << item->destX << item->destY << "move" << move << item->start;
	...
	if (item != m_draggedItem || !m_mouseIn) {
		...
		if (item->animation & MoveY) {
			if (y < item->destY) {
				y =  (item->start.y()+item->destY-item->start.y()) * move;
				if (y >= item->destY) {
					y = item->destY;
		...
		if (item->animation & MoveX) {
			if (x < item->destX) {
				x = item->start.x() + (item->destX-item->start.x()) * move;
				if (x >= item->destX) {
					x = item->destX;
...
void TaskbarLayout::updateCurrentTime(int current_time) {
	int   didAnimate  = None;
	int   willAnimate = None;

	qreal move = (qreal)current_time / m_expandDuration;
	qreal expand  = current_time * m_expandedWidth / m_expandDuration;
	
	qDebug() << "current_time" << current_time;
	foreach (TaskbarItem *item, m_items) 
	{
	  qDebug() << "item->animation" << item->animation;
	...
	qDebug() << "willAnimate" << willAnimate;
...
void TaskbarLayout::startAnimation() {
	if (m_animationsEnabled && state()!=Running) 
	{
