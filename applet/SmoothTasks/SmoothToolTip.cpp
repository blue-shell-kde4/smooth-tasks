...
bool SmoothToolTip::isAnimated()
{ 
  return m_scrollAnimation.state() != QAbstractAnimation::Stopped; 
}
...
void SmoothToolTip::moveBesideTaskItem(bool forceAnimated) {
	...
	if (pos == m_widget->pos() && isAnimated()) {
		if (forceAnimated) {
			m_shown = true;
			m_widget->show();
		}
...
void SmoothToolTip::startScrollAnimation(int dx, int dy, int duration) {
	...
	if (duration<=0) {
		...
		animationFinished();
                ...
                m_step = (qreal)1/duration;
		m_scrollAnimation.setDuration(duration);
		m_scrollAnimation.start();
	}
}

void SmoothToolTip::animationFinished() {
...
void SmoothToolTip::animate(int currentTime) {
	qreal progress = m_curve.valueForProgress(currentTime*m_step);
